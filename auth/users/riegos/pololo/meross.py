...
async def on_enchufe(email,passwd,name,num):
    try:
        ...
        await manager.async_init()
        await manager.async_device_discovery()
        #print("primero de todo vamos a buscar a nombre de:")
        #print(name)
        #all_devices = manager.find_devices()
        #print("todos miloco ")
        #print(all_devices)
        plugs = manager.find_devices(device_name=name)
        #print("lo que hemos encotnrado en meross es:")
        #print(plugs)
        if len(plugs) < 1:
...
async def off_enchufe(email,passwd,name,num):
    try:
        ...
        await manager.async_init()
        await manager.async_device_discovery()
        #print("primero vamos a buscar a nombre de:")
        #print(name)
        #all_devices = manager.find_devices()
        #print("todos miloco ")
        #print(all_devices)
...
async def check_status(email,passwd,type_dev):
    try:
        ...
        plugs = manager.find_devices(online_status=OnlineStatus.ONLINE)
        estado = {}
        if len(plugs) < 1:
            print("No plugs found...")
        else:
